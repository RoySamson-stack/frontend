...
import { AlertTriangle, Search, Calendar, Eye } from "lucide-react"
...
export default function AdminReportsPage() {
  ...
  const updateReportStatus = async (reportId: string, newStatus: string) => {
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`${endpoints.UPDATE_REPORT_STATUS}/${reportId}`, {
        method: "PATCH",
        headers: {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Review and manage all scam reports in the system.</p>
      </div>
